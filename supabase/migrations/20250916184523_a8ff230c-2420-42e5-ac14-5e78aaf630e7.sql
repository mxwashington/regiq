...
-- Recreate all functions with proper search path security
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "payment_logs_financial_isolation" ON public.payment_logs FOR ALL USING (user_id = auth.uid())';
  END IF;
END;
$$;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "api_keys_enhanced_security" ON public.api_keys FOR ALL USING (user_id = auth.uid() AND (rate_limit_per_hour > 0 OR usage_count >= 0))';
  END IF;
END;
$$;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "security_events_user_isolation" ON public.security_events FOR SELECT USING (user_id = auth.uid())';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "security_events_no_user_modifications" ON public.security_events FOR INSERT WITH CHECK (false)';
    EXECUTE 'CREATE POLICY "security_events_no_user_updates" ON public.security_events FOR UPDATE USING (false)';
    EXECUTE 'CREATE POLICY "security_events_no_user_deletes" ON public.security_events FOR DELETE USING (false)';
  END IF;
END;
$$;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    EXECUTE 'CREATE POLICY "admin_activities_admin_only" ON public.admin_activities FOR ALL USING (
      EXISTS(
...
BEGIN
  ...
  LEFT JOIN public.pg_policies p ON p.tablename = t.table_name AND p.schemaname = 'public'
