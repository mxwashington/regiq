-- Grant Admin Status and Highest Subscription to marcus@regiq.org (Fixed)
...
BEGIN
    ...
    IF marcus_user_id IS NOT NULL THEN
        ...
        -- Check if subscribers table exists and add enterprise subscription
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'subscribers' AND table_schema = 'public') THEN
            -- Add to subscribers table for enterprise access (using only existing columns)
...
BEGIN
    ...
    IF marcus_user_id IS NOT NULL THEN
        ...
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'user_entitlements' AND table_schema = 'public') THEN
