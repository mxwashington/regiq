-- CRITICAL SECURITY FIXES FOR REGIQ
-- Phase 1: Secure critical data tables with proper RLS policies
...
USING (
  ...
  log_sensitive_data_access('api_keys', 'ADMIN_ACCESS', 1, '["api_key", "user_id"]'::jsonb)
);

-- 3. SECURE TEAM_MEMBERS TABLE
-- Create secure policies for team members
...
-- 4. ENHANCE SECURITY EVENT MONITORING
-- Create security alert for admin access to sensitive data
CREATE OR REPLACE FUNCTION log_admin_sensitive_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log when admins access sensitive tables
  IF is_admin(auth.uid()) AND TG_TABLE_NAME IN ('api_keys', 'payment_logs', 'profiles') THEN
...
DROP TRIGGER IF EXISTS log_admin_access_api_keys ON public.api_keys;
CREATE TRIGGER log_admin_access_api_keys
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.api_keys
  FOR EACH ROW EXECUTE FUNCTION log_admin_sensitive_access();

DROP TRIGGER IF EXISTS log_admin_access_profiles ON public.profiles;
CREATE TRIGGER log_admin_access_profiles
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.profiles
  FOR EACH ROW EXECUTE FUNCTION log_admin_sensitive_access();

-- 5. SECURE PAYMENT LOGS ACCESS
-- Add additional security for payment logs
CREATE POLICY "secure_payment_logs_user_only" 
ON public.payment_logs 
FOR SELECT 
USING (
  auth.uid() = user_id AND 
  auth.jwt() IS NOT NULL AND
  log_sensitive_data_access('payment_logs', 'USER_ACCESS', 1, '["amount_cents", "stripe_session_id"]'::jsonb)
);
...
-- 7. CREATE SECURITY DASHBOARD FUNCTION
...
-- 9. CREATE SECURE SESSION MANAGEMENT
...
-- 10. FINAL SECURITY STATUS CHECK
SELECT check_current_security_status() as security_implementation_status;